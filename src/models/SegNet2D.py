...
def conv  (in_planes, out_planes, kernel_size=3, stride=1, activefun=nn.LeakyReLU(0.1, inplace=True)):
    return nn.Sequential(nn.Conv2d(in_planes, out_planes, 
                                   ...
                                   padding=(kernel_size - 1) // 2, bias=True), activefun)
...
def deconv(in_planes, out_planes, kernel_size=4, stride=2, activefun=nn.LeakyReLU(0.1, inplace=True)):
    return nn.Sequential(nn.ConvTranspose2d(in_planes, out_planes, 
                                            kernel_size=kernel_size, stride=stride, 
                                            padding=1, bias=True), activefun)


class SegNet2D(nn.Module):
    def __init__(self, num_imgs=2, num_segs=1):
        super(SegNet2D,self).__init__()

        self.activefun = nn.LeakyReLU(0.1, inplace=True)
        
        cps = [num_imgs*32, 128, 256, 512, 1024, 1024]
        dps = [64, 128, 256, 512, 1024]
        ...
        # DECODER 
        self.deconv5 = deconv(cps[5]         , dps[4], kernel_size=4, stride=2, activefun=self.activefun) 
        self.deconv5_1 = conv(dps[4] + cps[4], dps[4], kernel_size=3, stride=1, activefun=self.activefun) 

        self.deconv4 = deconv(cps[4]         , dps[3], kernel_size=4, stride=2, activefun=self.activefun) 
        self.deconv4_1 = conv(dps[3] + cps[3], dps[3], kernel_size=3, stride=1, activefun=self.activefun) 
    
        self.deconv3 = deconv(dps[3]         , dps[2], kernel_size=4, stride=2, activefun=self.activefun) 
        self.deconv3_1 = conv(dps[2] + cps[2], dps[2], kernel_size=3, stride=1, activefun=self.activefun) 

        self.deconv2 = deconv(dps[2]         , dps[1], kernel_size=4, stride=2, activefun=self.activefun) 
        self.deconv2_1 = conv(dps[1] + cps[1], dps[1], kernel_size=3, stride=1, activefun=self.activefun) 

        self.deconv1 = deconv(dps[1]         , dps[0], kernel_size=4, stride=2, activefun=self.activefun) 
        self.deconv1_1 = conv(dps[0] + cps[0], dps[0], kernel_size=3, stride=1, activefun=self.activefun) 

        self.last_conv = nn.Conv2d(dps[0], num_segs, kernel_size=3, stride=1, padding=1, bias=True)
...
def segnet2d(options, data=None):
    ...
    model = SegNet2D(num_imgs=options['segnet_num_imgs'],
                     num_segs=options['segnet_num_segs'])
